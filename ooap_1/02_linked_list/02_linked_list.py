...
class LinkedList(Generic[T]):
    """
    АТД связного списка с курсором.

    Пользователь работает только со списком и хранимыми значениями. Узлы и
    связи между ними являются скрытыми деталями реализации.
    ...
    def replace(self, value: T) -> None:
        # предусловие:
        # список не пустой (не существует случая, при котором список не пустой,
        # а курсор не указывает на какой-либо элемент)
        # постусловие при выполнении предусловия:
