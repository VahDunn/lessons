...
class BalancedBST:
    ...
    # чтобы тесты работали, доп. задания еще и здесь
    ...
    def _build(self, arr, low, high, parent, level):
        if low > high:
            return None
        ...
        node.LeftChild = self._build(arr, low, mid - 1, node, level + 1)
        node.RightChild = self._build(arr, mid + 1, high, node, level + 1)
        return node
